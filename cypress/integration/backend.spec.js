...
describe('Account Management Backend - Level 3', () => {
  ...
  it('creates new transaction', async () => {
    cy.request({
      ...
      headers: {
        "Content-Type": "application/json",
        "Transaction-Id": "54f485ab-885a-4124-b3f2-d8b290b727f5"
      },
      body: {
        account_id: "b5e66ed6-8e99-43e3-afed-ced56e3db426",
        amount: 10
      }
    }).then((response) => {
      expect(response.status).to.eq(200);
  ...
  it('returns transaction details', () =>{
    cy.request({
      method: 'GET',
      url: `${apiUrl}/transaction/54f485ab-885a-4124-b3f2-d8b290b727f5`,
    }).then((response) => {
      expect(response.status).to.eq(200);
      expect(response.body.account_id).to.eq("b5e66ed6-8e99-43e3-afed-ced56e3db426");
      expect(response.body.amount).to.eq(10);
    })
  })

  it('adds initial amount to account', async () => {
    cy.request({
      method: 'POST',
      url: `${apiUrl}/amount`,
      headers: {
        "Content-Type": "application/json",
        "Transaction-Id": "085325bb-fec6-4c7f-a8f8-324d789eaf86"
      },
      body: {
        account_id: "73be3749-4024-4be5-8611-b6715d01de8b",
        amount: 18
      }
    }).then((response) => {
      expect(response.status).to.eq(200);
  ...
  it('re-tris the same transaction', async () => {
    cy.request({
      method: 'POST',
      url: `${apiUrl}/amount`,
      headers: {
  ...
  it('only updates balance once due to idempotency', () =>{
    cy.request({
      method: 'GET',
      url: `${apiUrl}/balance/73be3749-4024-4be5-8611-b6715d01de8b`,
    }).then((response) => {
      expect(response.status).to.eq(200);
